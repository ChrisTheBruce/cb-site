...
import { setEmail as apiSetEmail, clearEmail as apiClearEmail, readEmailCookie, notifyDownload } from "@/components/Downloads";
...
export function useDlEmail() {
  ...
  const clear = React.useCallback(async () => {
    ...
    if ((resp as any).ok) setEmailState(null);
    else setError((resp as any).error || "Could not clear");
    setBusy(false);
    return resp;
  }, []);

  const notify = React.useCallback(async (filePathOrUrl: string, title?: string) => {
    // fire-and-forget is ok; but we still await here so caller can react to warn
